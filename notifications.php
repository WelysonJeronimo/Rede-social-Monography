                ...
                <div class="container-not">
                    <ul class="list-not">
                        ...
                        while ($noti = mysqli_fetch_array($result)) {
                            ...
                            $date = $noti['data_noti'];
                            $date = strtotime($date);
                            if ($noti['tipo_noti'] == 'comentou') {
                                if ($noti['lido'] == 0) {
                                    echo '<li class="notification" style="background-color:#bae3e1 !important;">
                                        <a href="viewposts.php?id='.$noti['id_item'].'&idn='.$noti['id_notificacoes'].'">
                                            <strong>' . $row['nome'] . ' ' . $noti['tipo_noti'] . ' em uma de suas publicações</strong>
                            ...
                            } elseif ($noti['tipo_noti'] == 'curtiu') {
                                if ($noti['lido'] == 0) {
                                    echo '<li class="notification" style="background-color:#bae3e1 !important">
                                        <a href="viewposts.php?id='.$noti['id_item'].'&idn='.$noti['id_notificacoes'].'">
                                            <strong>' . $row['nome'] . ' ' . $noti['tipo_noti'] . ' uma de suas publicações</strong>
                                            <p>' . date('d-m-Y H:i', $date) . '</p>
                                        </a>
                                    </li>';
                                } else {
                            ...
                            } elseif ($noti['tipo_noti'] == 'interesse') {
                                $sql = "SELECT * FROM ideias WHERE id_ideias={$noti['id_item']}";
                                $res = mysqli_query($connection,$sql);
                                $row2 = mysqli_fetch_assoc($res);
                                if ($noti['lido'] == 0) {
                                    echo '<li class="notification" style="background-color:#bae3e1 !important;">
                                        <a style="padding: 10px;">
                                            <strong>' . $row['nome'] . ' tem ' . $noti['tipo_noti'] . ' em sua ideia: '.$row2['titulo_ideia'].'</strong>
                                            <p>' . date('d-m-Y H:i', $date) . '</p>
                                        </a>
                                    </li>';
                                } else {
