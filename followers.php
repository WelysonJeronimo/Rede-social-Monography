    ...
    <section>
        <div class="container">
            <div class="followers">
                ...
                $query3 = "SELECT * FROM seguidores WHERE seguido={$user}";

                
                $result3 = mysqli_query($connection, $query3);

                while ($a = mysqli_fetch_array($result3)) {
                    $query = "SELECT * FROM usuarios WHERE id_usuario={$a['seguidor']}";
                    $query2 = "SELECT * FROM fotos_usuario WHERE id_usuario={$a['seguidor']}";
                    $result = mysqli_query($connection, $query);
                    $result2 = mysqli_query($connection, $query2);
                    $caminho = null;
                    echo '<div class="card">
                        <a href="visitProfile.php?id='.$a['seguidor'].'" style="display: flex;width: 100%;height:100%;justify-content: center;align-items: center;font-size: 15px;text-decoration: none;">
                            <div class="img">';
                            while ($c = mysqli_fetch_array($result2)) {
                                if ($a['seguidor'] == $c['id_usuario'] && $c['tipo_foto'] == 'perfil') {
                                    $caminho = $c['caminho'];
                                }
                            }
                            if ($caminho != null) {
